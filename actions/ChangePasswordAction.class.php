...
class users_ChangePasswordAction extends change_JSONAction 
{
	...
	public function _execute($context, $request)
	{
		...
		{
			return $this->sendJSONError($ls ->transBO('m.users.messages.error.allfieldsarerequired', array('ucf')));
		}
		
		// Test if new password are the same
		if ($newPwd != $newPwdConf)
		{
			return $this->sendJSONError($ls ->transBO('m.users.messages.error.newpasswordsnotsames', array('ucf')));
		}		
		...
		{
			return $this->sendJSONError($ls ->transBO('m.users.messages.error.badoldpassword', array('ucf')));	
		}
		
		$us->resetPassword($user, $newPwdConf);
		$succesmsg = $ls->transBO('m.users.bo.general.changepassword.successtext', array('ucf'), array('email' => $user->getEmail()));
		$this->logAction($user);
